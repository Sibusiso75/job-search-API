...
const SaveJob = require("../models/JobSeeker/Job/Job")
...
router.post("/save-job",verifyTokenAndAuthorization, async (req, res)=>{
 ...
 const certainJob = await Job.findOne()
      ...
      await newJobs.save()
      return res.json({ message:"Job has been saved successfully", newJobs})
  } catch (error) {
      console.log(error)
  }
})
router.get("/mysaveJobs", verifyTokenAndAuthorization, async (req, res)=>{
  try {
    
    // const user = await User.findOne({_id:req.user._id})
    // if(!user){
    //   return res.json({message:"You haven't saved any jobs yet"})
    // }
